...
* **Mono-Layout Spacing:** In the `mono-layout` component, horizontal (`@[hstack]`) and vertical (`@[vstack]`) spacing and alignment are natively controlled by passing specific CSS classes as attributes (e.g., `@[hstack](class: "gap-md center")`).

## Backend & Security (FastAPI)
* **Backend Responsibilities:** `src/server.py` handles real-time visual synchronization via Server-Sent Events (SSE) (`/api/sync/stream` and `/api/sync`) and stores user input data sent via HTTP POST (`/api/data`) in JSONL format. Start with `uv run server.py`.
