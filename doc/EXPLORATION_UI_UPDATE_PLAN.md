...
*   **実装方針:**
    ...
    *   **スタイリングの変更:** HUD モードが有効な場合、ドロワーのハンドル（開閉ボタン）を非表示にし、コンテナを常に画面上部（または指定位置）に `position: fixed` で固定します。ユーザーの操作を妨げないよう、`pointer-events` の調整や半透明な背景色などのスタイリングを検討します。
...
*   **構成ファイル:** `parser.py`, `script.js`, `style.css`, `template.html`
*   **実装方針:**
